...
<html lang="es">
  ...
  <body>
	...
	<div class="container">
	<form name="report" id="report" action="../<?php echo $user->getAbbreviation() ?>/generalZone.php" method="post" accept-charset="UTF-8"></form>
	<form name="savePDFForm" id="savePDFForm" class="form-signin" action="pdf/generateZonePDF.php" method="post" accept-charset="UTF-8">
	...
	<div class='text-center'><h2 class='form-signin-heading'><br />Reporte por zona de trayectorias escolares</h2></div><br />
...
while($ciclo < $numCiclo){
	...
	if($ciclo < 6){
		...
		$where = 'cohorte.grade = 1 AND cohorte.reprobate = 0 AND
			(s1.level = :schoolLevel AND s2.level = :schoolLevel AND
			s1.mode = :schoolMode AND s2.mode = :schoolMode AND
			s1.zone = :schoolZone AND s2.zone = :schoolZone)';
		$whereFields = array('schoolLevel' => $schoolLevel, 'schoolMode' => $schoolMode,
		...
		$where = 'cohorte.grade = 1 AND cohorte.reprobate = 0 AND
			(s.level = :schoolLevel AND s.mode = :schoolMode AND s.zone = :schoolZone)';
		$whereFields = array('schoolLevel' => $schoolLevel, 'schoolMode' => $schoolMode,
			'schoolZone' => $schoolZone);
		$students = $controller->displayBy2Action($where, $whereFields, $joinC, $showFields);
	}
