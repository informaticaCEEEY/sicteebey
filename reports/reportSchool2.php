...
if (!isset($_POST['cohorte']) && !isset($_POST['idSchool'])) {	
...
<html lang="es">
	...
	<div class='text-center'><h2 class='form-signin-heading'><br />Reporte por escuela de trayectorias escolares</h2></div>
	<br />
...
while($ciclo < $numCiclo){	
	...
	}elseif($school->getLevel() == 3){
		$whereFields= array('grade' => 7, 'reprobate' => 0, 'cct' => $school->getId(), 'cct2' => $school->getId());
		$joinC = 'INNER JOIN ' . $cohorte->getTableCohorte() . ' AS cohorte ON e.idStudent = cohorte.idStudent';
		$students = $controller->displayBy2Action($where, $whereFields, $joinC, $showFields);
	}	
