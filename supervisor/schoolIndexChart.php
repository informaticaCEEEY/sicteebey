...
<html lang="es">
	...
	<body>
		...
		<div class="container-fluid">
				...
				if($school[0]->getSchoolRegionZone() != $supervisorSchoolRegion[0]->getSchoolRegionZone()){
				    $_SESSION['message'] = 'La escuela no pertenece a su zona escolar';
				...
				?>
			<form name="schoolFactor" id="schoolFactor" action="contextSchoolDirector.php" method="post" accept-charset="UTF-8"></form>
		    ...
		    if($dataReportGeneral == FALSE){
		        $_SESSION['flash'] = 'La escuela con CCT: '. $school[0]->getCct() .' no se encuentra disponible por falta de datos';
			...
			<div class="col-xs-12 col-md-12">
				<div><h4 class='form-signin-heading'>CCT: <?php echo($school[0] -> getCct()); ?></h4></div>
				<div><h4 class='form-signin-heading'>Escuela: <?php echo($school[0] -> getName()); ?> </h4></div>
				<div><h4 class='form-signin-heading col-md-3 text-left'>Nivel: <?php echo($school[0] -> getSchoolLevelObject() -> getName()); ?> </h4></div>
				<div><h4 class='form-signin-heading col-md-3'>Modalidad: <?php echo($school[0] -> getSchoolModeObject() -> getName()); ?> </h4></div>
				<div><h4 class='form-signin-heading col-md-3'>Regi&oacute;n: <?php echo($school[0] -> getSchoolRegionObject() -> getName()); ?> </h4></div>
				<div><h4 class='form-signin-heading col-md-3'>Zona Escolar: <?php echo(str_pad($school[0]->getSchoolRegionZoneObject()->getZone(),  3, "0", STR_PAD_LEFT)); ?> </h4></div>
			</div>
			<div class="col-xs-12 col-md-12">
				<hr />
			</div>
			...
			<form role="form" name="entry" id="entry" class="form-signin" action="dispatchers/indexListDispatcher.php" method="post" accept-charset="UTF-8">
				<?php
				$form -> contextSchoolForm($school[0]->getCct());
