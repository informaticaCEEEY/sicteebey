...
$showFields = 'e.question, e.answer, count(e.answer) as total';
$where = 'factor = :factor';
$whereFields = array('factor' => $factorObject->getId());
$groupby = 'e.question, e.answer';
$factorStudentList = $factorStudentController -> displayBy2Action($where, $whereFields, '', '', $showFields, $groupby);
$factorStudentTotal = count($factorStudentList);
foreach ($factorStudentList as $factorStudentKey => $factorStudent) {
  $factorAnswerStudentsList[$factorStudent['question']][$factorStudent['answer']] = $factorStudent['total'];
}
