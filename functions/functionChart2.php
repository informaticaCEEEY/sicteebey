...
$showFields = 'school_region.name as region';
$join = "INNER JOIN school ON school.cct = e.cct
         ...
         INNER JOIN school_region ON school_region.id = school_region_zone.school_region";
$teacherInteractionList = $teacherInteractionController->displayByAction2('','',$join,'',$showFields);
$answerContext = array();
foreach($teacherInteractionList as $teacherInteraction){
    $answerContext[$teacherInteraction['id']]['region'][] = $teacherInteraction['region'];
    $answerContext[$teacherInteraction['id']]['answer'][] = $teacherInteraction['R8_R8'];
}
print_r($answerContext[1]);
exit;
