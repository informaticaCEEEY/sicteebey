...
$where = 'english_context.cct = :cct AND english_context.answered = :answered AND e.year = 2015';
...
<html lang="es">
 ...
 ?>
		...
		<div class="container-fluid">
			...
			$where = "cct LIKE :cct";
			$englishList = $englishController -> displayByAction($where, array('cct' => $school->getCct()));
			$englishTotal = count($englishList);
			
			$factorSchoolController = new FactorCctController();
			$whereCCT = "cct LIKE :cct AND factor = :factor";
			$factorSchoolList = $factorSchoolController -> displayByAction($whereCCT, array('cct' => $school->getCct(), 'factor' => 10));
			
			$factorRegionController = new FactorRegionController();
			$whereRegion = "region LIKE :region AND factor = :factor";
			$factorRegionList = $factorRegionController -> displayByAction($whereRegion, array('region' => $school->getRegionZone(), 'factor' => 10));
			
			$factorStateController = new FactorStateController();	
			$whereRegion = "factor LIKE :factor";		
			$factorStateList = $factorStateController -> displayByAction($whereRegion, array('factor' => 10));
