...
$where = 'bullying_context.cct = :cct AND bullying_context.answered = :answered AND e.year = 2015';
 ...
 ?>
		...
		<div class="container-fluid">
			...
			<div class="col-xs-12 col-md-7">
				<table class="table table-condensed">
					...
					foreach($idaepyProgArray[3] as $key => $value){
						// Tercero
						?>
						<!--form role="form" name="schoolGroup3<?php echo($key) ?>" id="schoolGroup3<?php echo($key) ?>" action="bullyingClassroom.php" method="post" accept-charset="UTF-8">
							<input type="hidden" id="cct" name="cct" value="<?php echo($school -> getCct()); ?>"/>
							<input type="hidden" id="grade" name="grade" value="3"/>
							<input type="hidden" id="group" name="group" value="<?php echo($key); ?>"/>
							<input type="hidden" id="factor" name="factor" value="<?php echo($factor); ?>"/>
						</form-->						
						<?php
						echo('<tr>');
						if($count3 == 1){							
						...
						//echo('<td><a onclick="document.forms.schoolGroup3'.$key.'.submit()">Ver grupo</a></td>');
						echo('</tr>');						
						$count3 += 1;
					}
			...
			<?php 
				...
				$where = "cct LIKE :cct";
				$bullyingList = $bullyingController -> displayByAction($where, array('cct' => $school -> getCct()));
				...
				$reportNameGeneral = array("0" => "Yucat\u00E1n", "1" => $school -> getSchoolRegionObject() -> getName(), 
                    2 =>"Zona " . str_pad($school->getSchoolRegionZoneObject()->getZone(),  3, "0", STR_PAD_LEFT), 3 => $school -> getCct());
                ...
                if($factorSchoolList[0]->getFactorCount() != 0){
                    ...
                    $dataReportGeneral .= "['" . $reportNameGeneral[2] . "'," . round($factorSchoolList[0] -> getMedia(), 2) . ",'" . $messageMediaNull . "', '']";
                }
				$dataReportGeneral .= ']';
			?>
