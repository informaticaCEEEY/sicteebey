<?php
class AprovModeDataController{

	private $model;

	function __construct(){

		$this->model=new AprovModeDataModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new AprovModeData();
			$object->setCohorte($cohorte);
			$object->setSchoolPeriod($schoolPeriod);
			$object->setMode($mode);
			$object->setTotal($total);
			$object->setNewStudents($newStudents);
			$object->setStudentsLeaving($studentsLeaving);
			$object->setTotalIdeal($totalIdeal);
			$object->setNewStudentsIdeal($newStudentsIdeal);
			$object->setStudentsIdealLeaving($studentsIdealLeaving);
			...
			$this -> model -> addAprovModeData($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setUnregistered($unregistered);
            $object->setUnregisteredThree($unregisteredThree);
			$this -> model -> updateAprovModeData($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteAprovModeData($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
