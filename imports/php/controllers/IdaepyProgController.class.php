<?php
class IdaepyProgController{

	private $model;

	function __construct(){

		$this->model=new IdaepyProgModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new IdaepyProg();
			$object->setCct($cct);
			$object->setGrade($grade);
			$object->setGroupSchool($groupSchool);
			$object->setTotal($total);
			$object->setYear($year);
			$this -> model -> addIdaepyProg($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setTotal($total);
			$object->setYear($year);
			$this -> model -> updateIdaepyProg($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteIdaepyProg($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
