<?php
class ContextApplicationController{

	private $model;

	function __construct(){

		$this->model=new ContextApplicationModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new ContextApplication();
			$object->setYearApplication($yearApplication);
			$this -> model -> addContextApplication($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			$object->setYearApplication($yearApplication);
			$this -> model -> updateContextApplication($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteContextApplication($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
