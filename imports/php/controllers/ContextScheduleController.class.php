<?php
class ContextScheduleController{

	private $model;

	function __construct(){

		$this->model=new ContextScheduleModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new ContextSchedule();
			$object->setCct($cct);
			$object->setGrade($grade);
			$object->setTotal($total);
			$object->setType($type);
			$object->setYear($year);
			$this -> model -> addContextSchedule($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setType($type);
			$object->setYear($year);
			$this -> model -> updateContextSchedule($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteContextSchedule($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
