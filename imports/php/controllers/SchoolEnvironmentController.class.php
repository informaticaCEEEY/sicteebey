<?php
class SchoolEnvironmentController{

	private $model;

	function __construct(){

		$this->model=new SchoolEnvironmentModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new SchoolEnvironment();
			$object->setStudent($student);
			$object->setCct($cct);
			$object->setR3_R3($R3_R3);
			$object->setR4_R4($R4_R4);
			$object->setR5_R5($R5_R5);
			$object->setR6_R6($R6_R6);
			$object->setR7_R7($R7_R7);
			$this -> model -> addSchoolEnvironment($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setR6_R6($R6_R6);
			$object->setR7_R7($R7_R7);
			$this -> model -> updateSchoolEnvironment($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteSchoolEnvironment($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
