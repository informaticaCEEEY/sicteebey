<?php
class PreescolarController{

	private $model;

	function __construct(){

		$this->model=new PreescolarModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new Preescolar();
			$object->setIdAlumno($idAlumno);
			$object->setNombre($nombre);
			$object->setApellidoPaterno($apellidoPaterno);
			$object->setAMaterno($aMaterno);
			$object->setCiclo($ciclo);
			$object->setCurp($curp);
			$object->setCct($cct);
			$object->setEscuela($escuela);
			$object->setGrado($grado);
			$object->setGrupo($grupo);
			$object->setTurno($turno);
			$object->setNivel($nivel);
			$object->setModalidad($modalidad);
			$object->setSector($sector);
			$object->setZona($zona);
			$object->setEstatus($estatus);
			$this -> model -> addPreescolar($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setZona($zona);
			$object->setEstatus($estatus);
			$this -> model -> updatePreescolar($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deletePreescolar($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
