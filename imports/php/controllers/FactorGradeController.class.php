<?php
class FactorGradeController{

	private $model;

	function __construct(){

		$this->model=new FactorGradeModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new FactorGrade();
			$object->setFactor($factor);
			$object->setGrade($grade);
			$this -> model -> addFactorGrade($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			$object->setFactor($factor);
			$object->setGrade($grade);
			$this -> model -> updateFactorGrade($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteFactorGrade($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
