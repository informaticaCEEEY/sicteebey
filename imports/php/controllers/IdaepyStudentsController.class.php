<?php
class IdaepyStudentsController{

	private $model;

	function __construct(){

		$this->model=new IdaepyStudentsModel();
	}
    ...
    public function countActives($where='', $whereFields='', $join=''){
        ...
        return $this->model->countActives($where, $whereFields, $join);
    }
	...
	public function createAction(){
		...
		try{
			...
			$object->setCct($cct);
			$object->setYear($year);
			$this -> model -> addIdaepyStudents($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setCct($cct);
			$object->setYear($year);
			$this -> model -> updateIdaepyStudents($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteIdaepyStudents($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
