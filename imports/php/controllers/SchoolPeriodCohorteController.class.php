<?php
class SchoolPeriodCohorteController{

	private $model;

	function __construct(){

		$this->model=new SchoolPeriodCohorteModel();
	}
	...
	public function getByAction($field, $search, $order=''){

		return $this->model->getBy($field, $search, $order);
	}
	...
	public function updateAction(){
		...
		try{
			...
			$object->setGrade($grade);
			$object->setSchoolLevel($schoolLevel);
			$this -> model -> updateSchoolPeriodCohorte($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteSchoolPeriodCohorte($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
