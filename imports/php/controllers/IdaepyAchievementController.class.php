<?php
class IdaepyAchievementController{

	private $model;

	function __construct(){

		$this->model=new IdaepyAchievementModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new IdaepyAchievement();
			$object->setStudent($student);
            $object->setPercentageHits($percentageHits);
			$object->setPercentageMath($percentageMath);
			$object->setPercentageScience($percentageScience);
			$object->setPercentageSpanish($percentageSpanish);
			$object->setAchievementMath($achievementMath);
			$object->setAchievementScience($achievementScience);
			$object->setAchievementSpanish($achievementSpanish);
			$object->setYear($year);
			$this -> model -> addIdaepyAchievement($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setAchievementSpanish($achievementSpanish);
			$object->setYear($year);
			$this -> model -> updateIdaepyAchievement($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteIdaepyAchievement($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
