<?php
class AchievementIdaepyController{

	private $model;

	function __construct(){

		$this->model=new AchievementIdaepyModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new AchievementIdaepy();
			$object->setAchievement($achievement);
			$object->setYear($year);
			$object->setOrderNumber($orderNumber);
			$object->setCodeR($codeR);
			$object->setCodeG($codeG);
			$object->setCodeB($codeB);
			$this -> model -> addAchievementIdaepy($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setCodeG($codeG);
			$object->setCodeB($codeB);
			$this -> model -> updateAchievementIdaepy($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteAchievementIdaepy($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
