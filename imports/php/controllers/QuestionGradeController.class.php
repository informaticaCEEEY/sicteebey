...
class QuestionGradeController{
	...
	function __construct(){
		...
		$this->model=new QuestionGradeModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new QuestionGrade();
			$object->setQuestion($question);
			$object->setGrade($grade);
			$object->setEnabled($enabled);
			$this -> model -> addQuestionGrade($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setGrade($grade);
			$object->setEnabled($enabled);
			$this -> model -> updateQuestionGrade($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteQuestionGrade($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
