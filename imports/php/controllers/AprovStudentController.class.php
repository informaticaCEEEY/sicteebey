<?php
class AprovStudentController{

	private $model;

	function __construct(){

		$this->model=new AprovStudentModel();
	}
	...
	public function displayByAction($where='', $whereFields='', $join='', $order='', $showFields=''){

		return $this->model->listAll('', '', '', '', $order, $where, $whereFields, $join, $showFields);
	}
	...
	public function getEntityByAction($field, $search, $order=''){

		return $this->model->getEntityBy($field, $search, $order);
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new AprovStudent();
			$object->setIdAprov($idAprov);
			$object->setIdStudent($idStudent);
			$object->setYear($year);
			$object->setGrade($grade);
			$object->setGroup1($group1);
			$object->setStatus($status);
			$object->setCct($cct);
			$object->setCalGlobal($calGlobal);
			$object->setReprobate($reprobate);
			$object->setGender($gender);
			$object->setSchoolMode($schoolMode);
			$this -> model -> addAprovStudent($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setGender($gender);
			$object->setSchoolMode($schoolMode);
			$this -> model -> updateAprovStudent($object);
		}catch(Exception $e){
			$_SESSION['message'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteAprovStudent($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
