<?php
class AprovModeSecuController{

	private $model;

	function __construct(){

		$this->model=new AprovModeSecuModel();
	}
    ...
    public function displayBy2Action($where='', $whereFields='', $join='', $order=''){

        return $this->model->listAll2('', '', '', '', $order, $where, $whereFields, $join);
    }
	...
	public function updateAction(){
		...
		try{
			...
			$object->setMode($mode);
			$object->setTotal($total);
			$this -> model -> updateAprovModeSecu($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteAprovModeSecu($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
