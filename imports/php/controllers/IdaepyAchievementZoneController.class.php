<?php
class IdaepyAchievementZoneController{

	private $model;

	function __construct(){

		$this->model=new IdaepyAchievementZoneModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new IdaepyAchievementZone();
			$object->setSchoolMode($schoolMode);
			$object->setSchoolZone($schoolZone);
			$object->setSubject($subject);
			$object->setAchievement($achievement);
			$object->setPercentage($percentage);
			$object->setYear($year);
			$this -> model -> addIdaepyAchievementZone($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setPercentage($percentage);
			$object->setYear($year);
			$this -> model -> updateIdaepyAchievementZone($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteIdaepyAchievementZone($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
