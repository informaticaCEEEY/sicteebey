<?php
class AprovGenderController{

	private $model;

	function __construct(){

		$this->model=new AprovGenderModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new AprovGender();
			$object->setCohorte($cohorte);
			$object->setSchoolPeriod($schoolPeriod);
			$object->setGrade($grade);
			$object->setTotalMen($totalMen);
			$object->setTotalWomen($totalWomen);
			$this -> model -> addAprovGender($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setTotalMen($totalMen);
			$object->setTotalWomen($totalWomen);
			$this -> model -> updateAprovGender($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteAprovGender($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
