<?php
class SchoolEnrollmentController{

	private $model;

	function __construct(){

		$this->model=new SchoolEnrollmentModel();
	}
	...
	public function createAction(){
		...
		try{
			...
			$object->setFinalScore($finalScore);
			$object->setIdCohorte($idCohorte);
			$object->setInitialCohort($initialCohort);
			$this -> model -> addSchoolEnrollment($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setIdCohorte($idCohorte);
			$object->setInitialCohort($initialCohort);
			$this -> model -> updateSchoolEnrollment($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteSchoolEnrollment($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
