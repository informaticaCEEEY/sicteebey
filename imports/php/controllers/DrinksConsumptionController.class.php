<?php
class DrinksConsumptionController{

	private $model;

	function __construct(){

		$this->model=new DrinksConsumptionModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new DrinksConsumption();
			$object->setStudent($student);
			$object->setCct($cct);
			$object->setR25_R34($R25_R34);
			$object->setR26_R35($R26_R35);
			$object->setR27_R36($R27_R36);
			$object->setR28_R37($R28_R37);
			$object->setR29_R38($R29_R38);
			$this -> model -> addDrinksConsumption($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setR28_R37($R28_R37);
			$object->setR29_R38($R29_R38);
			$this -> model -> updateDrinksConsumption($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteDrinksConsumption($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
