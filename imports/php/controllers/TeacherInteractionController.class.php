<?php
class TeacherInteractionController{

	private $model;

	function __construct(){

		$this->model=new TeacherInteractionModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new TeacherInteraction();
			$object->setStudent($student);
			$object->setCct($cct);
			$object->setR8_R8($R8_R8);
			$object->setR9_R9($R9_R9);
			$object->setR10_R12($R10_R12);
			$object->setR11_R17($R11_R17);
			$object->setR12_R19($R12_R19);
			$this -> model -> addTeacherInteraction($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setR11_R17($R11_R17);
			$object->setR12_R19($R12_R19);
			$this -> model -> updateTeacherInteraction($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteTeacherInteraction($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
