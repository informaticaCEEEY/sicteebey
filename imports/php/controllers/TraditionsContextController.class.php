<?php
class TraditionsContextController{

	private $model;

	function __construct(){

		$this->model=new TraditionsContextModel();
	}
	...
	public function createAction(){
		...
		try{
			...
			$this -> model -> addTraditionsContext($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setP11O4($P11O4);
			$object->setAnswered($answered);
			$this -> model -> updateTraditionsContext($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteTraditionsContext($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
