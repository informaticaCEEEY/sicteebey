<?php
class SchoolZoneHistorialController{

	private $model;

	function __construct(){

		$this->model=new SchoolZoneHistorialModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new SchoolZoneHistorial();
			$object->setCct($cct);
			$object->setSchoolRegionZone($schoolRegionZone);
			$object->setYear($year);
			$this -> model -> addSchoolZoneHistorial($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setSchoolRegionZone($schoolRegionZone);
			$object->setYear($year);
			$this -> model -> updateSchoolZoneHistorial($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteSchoolZoneHistorial($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
