<?php
class FoodConsumptionController{

	private $model;

	function __construct(){

		$this->model=new FoodConsumptionModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new FoodConsumption();
			$object->setStudent($student);
			$object->setCct($cct);
			$object->setR17_R26($R17_R26);
			$object->setR18_R27($R18_R27);
			$object->setR19_R28($R19_R28);
			$object->setR20_R29($R20_R29);
			$object->setR21_R30($R21_R30);
			$object->setR22_R31($R22_R31);
			$object->setR23_R32($R23_R32);
			$object->setR24_R33($R24_R33);
			$this -> model -> addFoodConsumption($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setR23_R32($R23_R32);
			$object->setR24_R33($R24_R33);
			$this -> model -> updateFoodConsumption($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteFoodConsumption($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
