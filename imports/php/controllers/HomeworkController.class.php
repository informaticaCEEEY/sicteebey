...
class HomeworkController{
	...
	public function createAction(){
		...
		try{
			...
			$object->setCct($cct);
			$object->setR51_R73($R51_R73);
			$object->setR52_R74($R52_R74);
			$object->setR53_R75($R53_R75);
			$object->setR54_R76($R54_R76);
			$object->setR55_R77($R55_R77);
			$object->setR56_R78($R56_R78);
			$this -> model -> addHomework($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setR55_R77($R55_R77);
			$object->setR56_R78($R56_R78);
			$this -> model -> updateHomework($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteHomework($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
