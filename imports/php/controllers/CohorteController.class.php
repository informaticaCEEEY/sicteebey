...
class CohorteController{
	...
	public function displayByCohorte($cohorte){

		return $this->model->listAll('', '', '', '', '', $cohorte);
	}
	...
	public function getByCohorteAction($cohorte){

		return $this->model->getByCohorte($cohorte);
	}
	...
	public function createAction(){
		...
		try{
			$object= new Cohorte();
			$object->setName($name);
			$object->setTableCohorte($tableCohorte);
			$this -> model -> addCohorte($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			$object->setName($name);
			$object->setTableCohorte($tableCohorte);
			$this -> model -> updateCohorte($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteCohorte($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
