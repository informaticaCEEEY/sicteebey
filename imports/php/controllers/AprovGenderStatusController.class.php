<?php
class AprovGenderStatusController{

	private $model;

	function __construct(){

		$this->model=new AprovGenderStatusModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new AprovGenderStatus();
			$object->setCohorte($cohorte);
			$object->setSchoolPeriod($schoolPeriod);
			$object->setGender($gender);
			$object->setStatusA($statusA);
			$object->setStatusR($statusR);
			$object->setStatusX($statusX);
			$object->setStatusZ($statusZ);
			$object->setUnregisteredThree($unregisteredThree);
			$this -> model -> addAprovGenderStatus($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setStatusZ($statusZ);
			$object->setUnregisteredThree($unregisteredThree);
			$this -> model -> updateAprovGenderStatus($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteAprovGenderStatus($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
