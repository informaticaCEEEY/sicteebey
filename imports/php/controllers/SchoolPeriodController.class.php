...
class SchoolPeriodController{
	...
	function __construct(){
		...
		$this->model=new SchoolPeriodModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new SchoolPeriod();
			$object->setName($name);
			$object->setTablePeriod($tablePeriod);
			$object->setStartYear($startYear);
			$object->setEndYear($endYear);
			$this -> model -> addSchoolPeriod($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setStartYear($startYear);
			$object->setEndYear($endYear);
			$this -> model -> updateSchoolPeriod($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteSchoolPeriod($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
