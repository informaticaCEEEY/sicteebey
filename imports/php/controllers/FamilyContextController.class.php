<?php
class FamilyContextController{

	private $model;

	function __construct(){

		$this->model=new FamilyContextModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new FamilyContext();
			$object->setStudent($student);
			$object->setCct($cct);
			$object->setR13_R20($R13_R20);
			$object->setR14_R21($R14_R21);
			$object->setR15_R24($R15_R24);
			$object->setR16_R25($R16_R25);
			$object->setR40_R62($R40_R62);
			$this -> model -> addFamilyContext($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			...
			$object->setR16_R25($R16_R25);
			$object->setR40_R62($R40_R62);
			$this -> model -> updateFamilyContext($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteFamilyContext($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
