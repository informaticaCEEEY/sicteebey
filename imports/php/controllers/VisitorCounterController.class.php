...
class VisitorCounterController{
	...
	function __construct(){
		...
		$this->model=new VisitorCounterModel();
	}
	...
	public function createAction(){
		...
		extract($_POST);
		try{
			$object= new VisitorCounter();
			$object->setVisitDate($visitDate);
			$object->setIpAddress($ipAddress);
			$this -> model -> addVisitorCounter($object);
		}catch(Exception $e){
	...
	public function updateAction(){
		...
		try{
			$object->setVisitDate($visitDate);
			$object->setIpAddress($ipAddress);
			$this -> model -> updateVisitorCounter($object);
		}catch(Exception $e){
			$_SESSION['flash'] = $e->getMessage();
			echo('<script>javascript:history.go(-1)</script>');
		}
	...
	public function deleteAction(){
		...
		if(isset($form_id) && isset($id) && is_numeric($form_id) && is_numeric($id) && $id==$form_id){
			...
			if($object != null){
				...
				$this -> model -> deleteVisitorCounter($object);
			}
		}else{
			echo('<form name="valid" id="valid" action="../index.php" method="post"></form>');
			echo('<script>document.forms.valid.submit()</script>');
