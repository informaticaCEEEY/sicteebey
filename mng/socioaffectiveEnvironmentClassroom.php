...
if(isset($_POST['groupby'])){
  ...
  $join = 'INNER JOIN socioaffective_environment on socioaffective_environment.student = e.student ';
  $join .= 'INNER JOIN idaepy_students on idaepy_students.student = socioaffective_environment.student ';
  $where = 'socioaffective_environment.answered = :answered AND idaepy_students.cct = :cct AND idaepy_students.grade = :grade
  ...
  $whereFields = array('cct' => $school -> getCct(), 'grade' => $idaepyScheduled->getGrade(),
                  'schoolGroup' => $idaepyScheduled->getSchoolGroup(), 'year' => $groupby);
  $socioaffectiveEnvironmentList = $socioaffectiveEnvironmentController -> displayBy2Action($where, $whereFields, $join);
  $factorTotal = count($socioaffectiveEnvironmentList);
}else{
  $socioaffectiveEnvironmentController = new SocioaffectiveEnvironmentController();
        ...
        <div class="container-fluid">
          ...
          <button class="buttonBack" type="button" onclick="document.forms.backPage.submit()"><span>Regresar</span></button>
            ...
            for ($i = 1; $i <= count($socioaffectiveEnvironmentName1); $i++) {
                ...
                $socioaffectiveEnvironmentPercent1 = round(($socioaffectiveEnvironmentTotalAnswers[1][$i] / $factorTotal * 100), 2);
            ...
            for ($i = 1; $i <= count($socioaffectiveEnvironmentName1); $i++) {
                ...
                $socioaffectiveEnvironmentPercent2 = round(($socioaffectiveEnvironmentTotalAnswers[2][$i] / $factorTotal * 100), 2);
            ...
            for ($i = 1; $i <= count($socioaffectiveEnvironmentName1); $i++) {
                ...
                $socioaffectiveEnvironmentPercent3 = round(($socioaffectiveEnvironmentTotalAnswers[3][$i] / $factorTotal * 100), 2);
            ...
            for ($i = 1; $i <= count($socioaffectiveEnvironmentName1); $i++) {
                ...
                $socioaffectiveEnvironmentPercent4 = round(($socioaffectiveEnvironmentTotalAnswers[4][$i] / $factorTotal * 100), 2);
